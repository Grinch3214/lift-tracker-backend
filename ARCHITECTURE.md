...
**Важно для клиента:** признак "миграция прошла успешно, можно чистить `localStorage`" — это **`rejected: []`** в ответе, а не просто `HTTP 200`. Даже для самой первой регистрации фронт обязан явно проверять пустоту `rejected`, а не полагаться на факт получения ответа — сетевые/валидационные ошибки на уровне HTTP всё ещё возможны независимо от LWW.
