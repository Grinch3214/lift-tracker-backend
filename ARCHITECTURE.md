...
- **`pg`** (сырой SQL) — на старте, сознательно без ORM.

Причина: цель — сначала понять, что происходит на уровне SQL (таблицы и связи, `JOIN`, индексы, `INSERT`/`UPDATE`/`DELETE`, транзакции, constraints, миграции), и только потом подключить ORM (Prisma/TypeORM/Drizzle — не выбрано) и сравнить, как те же операции выглядят через неё. ORM-слой в этот документ пока не входит — будет отдельным приложением поверх того, что здесь описано, когда до него дойдёт очередь.
...
- **Один email, два способа входа.** Что происходит, если пользователь зарегистрировался через email+пароль, а потом впервые логинится через Google с тем же адресом (или наоборот)? Слить в один `users`-ряд по email, или это два разных пользователя? Решить перед реализацией `/auth/google/callback`.
- **Синхронизация soft-deleted записей.** `custom_exercises`/`custom_muscle_groups` с `is_deleted=true` — синхронизировать их тоже (чтобы удаление на одном устройстве применилось и на другом), почти наверняка да, но нужно явно прописать в реализации `push`/`pull`, а не забыть по умолчанию отфильтровать.
- **Миграции.** Пронумерованные `.sql`-файлы + раннер (например `node-pg-migrate`) — не ORM-миграции, т.к. ORM пока не подключена. Конкретный инструмент не выбран.
- **ORM-слой.** Не описан в этом документе намеренно — будет отдельным приложением/разделом, когда дойдёт очередь сравнить с сырым SQL (Prisma vs TypeORM vs Drizzle — не выбрано).
